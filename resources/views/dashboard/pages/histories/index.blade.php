...
@section('content')
    <header class="header">
        ...
        <ul class="header-navigation">
            <li class="header-navigation-item">
                <a class="header-navigation-link current" href="{{route('dashboard.histories')}}">Истории: {{$historiesQuantity}}</a>
            </li>
            <li class="header-navigation-item">
    ...
    <ul class="breadcrumbs book-read-page__breadcrumbs">
        <li class="breadcrumbs-item">
            <a class="breadcrumbs-link current" href="{{route('dashboard.histories')}}">Истории</a>
        </li>
    </ul>
    <main class="histories-page">
        @if ($histories && $histories->count() > 0)
            <ul class="list">
                @foreach ($histories as $history)
                    <li class="list-item">
                        <span class="list-item-title">{{$rank++}}. ({{$history->year}}) {{$history->title}}</span>
                        <a class="list-action" href="{{route('dashboard.histories.update', $history->id)}}">
                        ...
                        <button class="list-action red-bg" type="button" data-action="delete-history" data-history="{{$history->id}}">

...
@section('content')
    <header class="header">
        ...
        <ul class="header-navigation">
            <li class="header-navigation-item">
                <a class="header-navigation-link current" href="{{route('dashboard.news')}}">Новости: {{$newsQuantity}}</a>
            </li>
            <li class="header-navigation-item">
                <a class="header-navigation-link" href="{{route('dashboard.news.categories')}}">Категории: {{$categoriesQuantity}}</a>
            </li>
            <li class="header-navigation-item">
    ...
    <ul class="breadcrumbs book-read-page__breadcrumbs">
        <li class="breadcrumbs-item">
            <a class="breadcrumbs-link current" href="{{route('dashboard.news')}}">Новости</a>
        </li>
    </ul>
    <main class="news-page">
        @if ($news && $news->count() > 0)
            <ul class="list">
                @foreach ($news as $new)
                    <li class="list-item">
                        <span class="list-item-title">{{$rank++}}. {{$new->title}}</span>
                        <a class="list-action" href="{{route('dashboard.news.update', $new->id)}}">
                        ...
                        <button class="list-action red-bg" type="button" data-action="delete-news" data-news="{{$new->id}}">
